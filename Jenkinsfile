...
"""
   ...
   stages {
     stage('Deploy') {
		 ...
		 secret_map = httpRequest contentType: 'APPLICATION_JSON', customHeaders: [[maskValue: false, name: 'X-Vault-Token', value: 'abc123']], httpMode: 'POST', ignoreSslErrors: true, requestBody: '''{ "metadata": " "dc": "${dc}",  "gitorg": "${gitorg}", "appname": "${appname}"}"}''', responseHandle: 'NONE', url: 'http://34.69.161.191/v1/auth/${gitorg}/${appname}/role/${dc}/secret-id', wrapAsMultipart: false
